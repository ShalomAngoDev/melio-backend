...
export class ChatAIService {
  ...
  private readonly emotionKeywords = {
    fear: ['peur', 'peur', 'effrayé', 'effrayée', 'terrifié', 'terrifiée', 'anxiété', 'anxieux', 'anxieuse', 'stress', 'stressé', 'stressée'],
    sadness: ['triste', 'tristesse', 'pleurer', 'pleure', 'déprimé', 'déprimée', 'malheureux', 'malheureuse', 'désespéré', 'désespérée'],
    anger: ['colère', 'énervé', 'énervée', 'fâché', 'fâchée', 'rage', 'frustré', 'frustrée', 'furieux', 'furieuse'],
    loneliness: ['seul', 'seule', 'isolement', 'isolé', 'isolée', 'personne', 'ami', 'amie', 'copain', 'copine'],
    bullying: ['moquer', 'moqueries', 'insulte', 'insultes', 'harcèlement', 'harceler', 'embêter', 'embêté', 'embêtée', 'taquiner', 'taquine'],
    violence: ['frapper', 'frapper', 'battre', 'battu', 'battue', 'violence', 'violent', 'violente', 'agression', 'agresser'],
    school: ['école', 'classe', 'professeur', 'prof', 'cours', 'devoir', 'devoirs', 'examen', 'examens', 'notes'],
  ...
  analyzeAndRespond(userMessage: string): ChatResponse {
    ...
    this.logger.log(`Chat AI - Catégorie détectée: ${mainCategory}, Message: "${userMessage.substring(0, 50)}..."`);
  ...
  private selectMainCategory(categories: string[]): string {
    ...
    const priority = ['violence', 'bullying', 'fear', 'sadness', 'anger', 'loneliness', 'school', 'family'];
  ...
  private generateResponse(category: string, message: string): ChatResponse {
    ...
    const encouragement = shouldAddEncouragement ? 
      this.responses.encouragement[Math.floor(Math.random() * this.responses.encouragement.length)] : '';
    ...
    if (resourceId && !finalResponse.includes('montrer')) {
      const resourceSuggestion = this.responses.resources[Math.floor(Math.random() * this.responses.resources.length)];
  ...
  isOffTopic(message: string): boolean {
    ...
    const offTopicKeywords = [
      'politique', 'élection', 'vote', 'gouvernement',
      'sexe', 'sexuel', 'sexuelle', 'adultes',
      'drogue', 'alcool', 'cigarette',
      'argent', 'travail', 'emploi',
      'religion', 'dieu', 'prière'
    ];
